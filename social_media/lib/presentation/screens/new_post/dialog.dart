                ...
                return BlocConsumer<UploadPostBloc, UploadPostState>(
                  listener: (context, state) {
                    if (state is UploadPostSuccess) {
                      ...
                      bool alreadyFoundInGlobalfeed = false;
                      ...
                      for (PostModel post in posts) {
                        ...
                        if (homeState is HomeFeedSuccess) {
                          for (HomeFeedModel model in homeState.homeFeed) {
                            if (model.post.postId == state.postModel.postId) {
                              alreadyFoundInGlobalfeed = true;
                              break;
                            }
                          }
                        }

                        if (!alreadyFoundInGlobalfeed) {
                          if (homeState is HomeFeedSuccess) {
                            homeState.homeFeed.insert(
                              0,
                              HomeFeedModel(
                                  post: state.postModel,
                                  user: (profileState as ProfileSuccess)
                                      .profileModel
                                      .user),
                            );
                          }
